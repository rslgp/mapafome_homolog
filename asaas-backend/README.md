...
| `api/asaas/health.js` | GET — liveness probe. Returns `{ ok, service, env, apiKey: "set"\|"missing", time }`. Does **not** call Asaas (no quota cost) and never leaks the key — safe for an uptime monitor / load balancer. |
