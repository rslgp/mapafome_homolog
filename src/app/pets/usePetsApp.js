...
// usePetsApp, o CÉREBRO do /pets, extraído do PetsApp (M10a).
//
// PORQUE ESTE HOOK EXISTE
//   PetsApp era um god-component: ~20 useState, muitos useCallback, os useMemo
//   derivados, os dois useEffect (GPS + carga; flush da fila offline) e o wiring
//   de mapa/lista/sheets num só corpo de função de ~590 LOC. O M10a decompõe esse
//   corpo em DUAS peças por responsabilidade (SRP): TODA a lógica de estado +
//   handlers + derivações vive AQUI, num hook customizado testável isoladamente;
//   o PetsApp.js volta a ser um componente FINO que chama const {...} = usePetsApp()
//   e só renderiza o JSX. É uma extração que PRESERVA COMPORTAMENTO: o MESMO estado,
//   os MESMOS handlers, a MESMA saída de render, só REALOCADOS. Nada de lógica,
//   handler ou saída de render mudou; os comentários PET-M* originais viajam com o
...
import { useLocale } from '../components/compatibility/components/ux/strings';
...
export default function usePetsApp() {
  ...
  // A superfície de estado + handlers + derivações que o PetsApp.js (componente
  // fino) consome. A MESMA forma que estava em escopo no corpo do god-component,
  // só que agora devolvida por um hook, tornando a lógica testável isoladamente.
  return {
    // estado + derivados que o render lê
    center,
    hasGps,
    view,
    reportOpen,
    ...
    isReady,
    showMap,
    // handlers que o render liga aos filhos
    handleRetryLoad,
