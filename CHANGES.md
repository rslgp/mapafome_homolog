...
> **LAST_UPDATED:** 2026-07-09 · branch `loop/mapafome` · **PASS 13 rodado** (invocado explicitamente via /milestone-mapafome-expand; quota pre-pass sessao 21% | semana 17% — folga confortavel). +4 itens em 4 areas novas, TODOS commitaveis (nenhum human-gate). Estado: **167 itens/88 areas/13 passes** (validado por parser). Achado que dói: EXT-READROW-01 (S) — o path de leitura de fome faz JSON.parse por linha SEM try/catch por-linha, 1 celula malformada num Sheet colaborativo derruba o mapa inteiro pra todo visitante (o /pets ja pula a linha ruim). **Top commitavel segue EXT-DBLSUBMIT-01** (S+, fix pequeno e testavel), depois EXT-READROW-01 e EXT-REP2-01. Taxa de vazio ~69% — o codigo maduro se defende sozinho; valor restante = IMPLEMENTAR, nao escanear.
...
> **ROADMAPS:** `ROADMAP_VERTENTES.yaml` (multi-vertente, 30 itens) · `MILESTONES_EXTENDED.yaml` (gap-scan ativo no loop, 167 itens/88 areas/13 passes, 5 tiers S+/S/S-/A+/A) · `UIUX_MILESTONES.yaml` (UI/UX) · `MILESTONES.yaml` (P-series/pagamento). Gate SOT em `CLAUDE.md`.
