...
> **LAST_UPDATED:** 2026-07-10 · branch `loop/mapafome` · **ONDA 3 IMPLEMENTADA E SHIPPED** (5 commits atomicos d9fbbab..7f4fd51, gate 100% verde LIDO: lint/test 1445/fitness/build 13/smoke200 16-16). 6 milestones: EXT-A11Y-01+FOCUSTRAP-01 (S-, useFocusTrap trava Tab nos sheets aria-modal), EXT-I18N-04 (S-, 4 msgs de falha de pagamento -> t() 12 locales), EXT-CURRENCY-01/02 (S-, erros de validacao -> t() + doc BRL-only), EXT-HREFLANG-01 (S-, og:locale:alternate). Corrigido um build-break do trabalho parcial (import de SUPPORTED_LOCALES de engine.js 'use client' pra structuredData.js server quebrava /imprensa -> extraido pra locales.js puro) + bug do focus-trap (button[tabindex=-1] entrava no ciclo) + re-ancora FF2 de 3 sheets. ARIALIVE/I18N-03/SHARELOC SEGURADOS (tocam strings.page.js sujo da concorrente). CPF-condicional e hreflang-por-rota HUMAN-GATED. Estado: **168 itens/89 areas · shipped 15 / pending 131**. Proximo: ONDA 4 (corretude/infra/map perf) — ver PRIORIDADES.md.
...
- [`2026-07-10-AD`](#2026-07-10-ad--onda-3-shipped-6-milestones--5-commits) — **ONDA 3 SHIPPED** (a11y/i18n ultimo-metro): 6 milestones via 3 agents paralelos + 5 commits atomicos (d9fbbab..7f4fd51), gate 100% verde LIDO (lint/test 1445/fitness/build 13/smoke200 16-16). EXT-A11Y-01+FOCUSTRAP-01 (S-, useFocusTrap compartilhado trava Tab/Shift+Tab nos sheets aria-modal — re-query por keydown, tabindex=-1 excluido, 7/7 tests), EXT-I18N-04 (S-, 4 msgs de falha de pagamento hardcoded pt-BR -> t() nos 12 locales em strings.assinar.js), EXT-CURRENCY-01 (S-, comentario BRL-only-by-design), EXT-CURRENCY-02 (S-, erros de validacao -> t(); CPF-condicional HUMAN-GATED), EXT-HREFLANG-01 (S-, og:locale:alternate; hreflang-por-rota HUMAN-GATED). **Recuperacao de tree quebrado:** os 3 agents foram PARADOS mid-work; o parcial quebrou o build (import de SUPPORTED_LOCALES de engine.js 'use client' pra dentro do structuredData.js server -> `.filter is not a function` em /imprensa) — corrigido extraindo pra locales.js React-free (engine re-exporta, consumidores intactos); + bug real do focus-trap (button[tabindex=-1] entrava no ciclo como phantom last) corrigido; + re-ancora FF2 de 3 sheets deslocados pelo wiring. **SEGURADOS** (tocam strings.page.js sujo da concorrente): ARIALIVE-01, I18N-03, SHARELOC-01. Nenhum arquivo da concorrente commitado (hash-object 3 arquivos UNCHANGED). shipped 9->15, pending 137->131.
- [`2026-07-09-AC`](#2026-07-09-ac--onda-12-shipped-6-fixes--7-commits) — **ONDA 1+2 SHIPPED** (implementacao, nao scan): 6 fixes do backlog implementados via 5 agents paralelos (arquivos disjuntos) + 7 commits atomicos (e39cf61..d4f857a), gate 100% verde LIDO (lint/test 1433/fitness/build 13/smoke200 16-16/backend 112). EXT-DBLSUBMIT-01 (S+, isPublishing flag + guard de re-entrada em handleClickMap — o botao de publicar fome nunca ficava disabled, double-tap gravava 2 pontos; ramo morto {false ?} em MainControls.js:325 revivido), EXT-READROW-01 (S, mapFoodRow com parse por-linha em try/catch pula a linha ruim espelhando /pets), EXT-CSV-01 (S-, csvEsc prefixa ' em =/+/-/@, CWE-1236), EXT-TIMEOUT-01+FORM-01 (S-, AbortController 12s + MIN_PHONE_DIGITS=8 no client de doacao), EXT-SEC-04 (S-, throttle per-IP 429 no create-subscription, store dual memory/Upstash fail-OPEN), EXT-EH-05 (S-, seam resolveDadosWithCoordinates + return antes do addRow, sem ghost pin). +1 chore(fitness) re-ancorou o FF2 baseline App.js:136->:133 (a sessao concorrente deletou 3 linhas acima do constructor, deslocando a ancora line-anchored e tornando o MESMO debt ja-allowlistado red). EXT-REP2-01 (S) confirmado ja-shipped (af3b693). Nenhum arquivo da sessao concorrente tocado (provado por git log --name-only + hash-object). shipped 2->9, pending 144->137.
...
- **168 milestones** em **89 areas** · pending **131** · blocked-human **19** · later **3** (cross-refs) · **shipped 15**. Validado via parser YAML: 0 ids duplicados.
- **SHIPPED ate agora (15):** ONDA 1+2 (7 commits): DBLSUBMIT-01, READROW-01, CSV-01, TIMEOUT-01, FORM-01, SEC-04, EH-05 · ONDA 3 (5 commits): A11Y-01, FOCUSTRAP-01, I18N-04, CURRENCY-01, CURRENCY-02, HREFLANG-01 · REP2-01 (af3b693). SEC-02/FF11 shipped fora do arquivo.
- Por tier: **S+ 2** (EXT-SEC-01 CVE next 16.2.4 + EXT-DBLSUBMIT-01 ja SHIPPED) · **S 11** · S- 55 · A+ 47 · A 53.
- 1 achado ja shipped fora do arquivo: SEC-02/FF11 (gate mecanico de secret-leak no bundle, allowlist por hash).
- **ONDA 1+2 SHIPPED 2026-07-09 (7 commits e39cf61..d4f857a, gate verde lido):** EXT-DBLSUBMIT-01 (S+, guard double-submit), EXT-READROW-01 (S, parse por-linha resiliente), EXT-CSV-01 (S-, formula-injection), EXT-TIMEOUT-01+EXT-FORM-01 (S-, timeout + telefone na doacao), EXT-SEC-04 (S-, rate-limit per-IP), EXT-EH-05 (S-, sem ghost pin). EXT-REP2-01 (S) confirmado ja-shipped (af3b693). +1 chore(fitness) re-ancorou o FF2 baseline (App.js:136->:133, deslocado pela sessao concorrente).
...
- **Proximo a pegar (commitavel, sem human_gate, sem deps):** ONDA 3 (a11y/i18n ultimo-metro): EXT-ARIALIVE-01, EXT-A11Y-01/FOCUSTRAP-01, EXT-I18N-03/04, EXT-SHARELOC-01, EXT-CURRENCY-01/02, EXT-HREFLANG-01. Depois ONDA 4 (corretude/infra/map perf: EXT-MAP-01/02, EXT-BOOTGUARD-01, EXT-WEBHOOKIDEM-01, EXT-LEGEND-01, EXT-EH-02). Ver PRIORIDADES.md.
...
## 2026-07-09-AC — ONDA 1+2 shipped (6 fixes, 7 commits)

**Comando:** "faca todos os fixes" -> escopo confirmado ONDA 1+2 (recommended). Primeira sessao de IMPLEMENTACAO depois de 14 passes de scan (saturado ~90% vazio).

**Metodo:** DBLSUBMIT-01 implementado antes (via agent_principalengineer). Os outros 6 milestones roteados a **5 agents principalengineer em PARALELO**, agrupados por arquivo pra garantir arquivos DISJUNTOS (sem conflito): (A) READROW appMainBootstrap.js, (B) CSV csv.js + REP2 reports.js, (C) FORM+TIMEOUT asaasSubscriptionClient.js, (D) SEC-04 asaas-backend, (E) EH-05 endereco.js. Cada agent rodou so o test do proprio modulo (barato); build/smoke200/fitness SERIALIZADOS 1x no fim (evita OOM de builds paralelos — licao no-parallel-build-agents).

| id | tier | fix | arquivos | commit |
|---|---|---|---|---|
| EXT-DBLSUBMIT-01 | S+ | flag `isPublishing` desabilita o botao + `CircularProgress` (ramo morto `{false ?}` MainControls.js:325 revivido) + guard de re-entrada em handleClickMap cobrindo path sync E async geofence | App.js, AppMapGrid.js, MainControls.js, +test | 4542763 |
| EXT-READROW-01 | S | `mapFoodRow(x,aes)`: parse de Dados/Coordinates por-linha em try/catch, pula a linha ruim (espelha /pets parsePetRow); +13 testes | appMainBootstrap.js, +test | 8a38957 |
| EXT-CSV-01 | S- | `csvEsc` prefixa `'` quando o valor comeca com =/+/-/@ (CWE-1236), antes do quoting; +6 testes | ux/csv.js, +test | 387cb2f |
| EXT-TIMEOUT-01 | S- | AbortController 12s no requestWithFallback (createSubscription + fetchSubscriptionPayment); timeout cai no retry-envelope existente | payments/asaasSubscriptionClient.js, +test | a8488f6 |
| EXT-FORM-01 | S- | `MIN_PHONE_DIGITS=8` floor generico (nao BR-mask) em validateBeforeSubmit; aceita mobilePhone/telefone | (mesmo arquivo/commit do TIMEOUT-01) | a8488f6 |
| EXT-SEC-04 | S- | throttle per-IP fixed-window (429 + Retry-After) no create-subscription; store dual memory/Upstash espelhando idempotencyStore; **fail-OPEN deliberado** (oposto do webhook fail-closed — rate-limit e controle de abuso, nao de correcao: KV down nao pode virar donation outage) | asaas-backend: create-subscription.js, lib/http.js (clientIp), lib/rateLimitStore.js, +2 test | 7a71135 |
| EXT-EH-05 | S- | seam puro `resolveDadosWithCoordinates`; catch alerta + limpa spinner + `return` ANTES do addRow (sem ghost pin); success path byte-identico | googlesheets/endereco.js, enderecoGeocode.js, +2 test | d4f857a |

**+1 chore(fitness) — commit e39cf61 (habilitador):** a sessao concorrente deletou 3 linhas acima do constructor do App.js, deslocando a ancora line-anchored do FF2_BASELINE de :136 pra :133; o MESMO constructor ja-allowlistado (debt pre-existente) virou red por a entrada nao bater mais a linha. Re-ancorado :136->:133 (mesmo debt, ancora atualizada — padrao ja documentado no proprio comentario do baseline). Commitado ANTES do App.js (4542763) pro fix nao commitar contra gate red. **Circuit-breaker aplicado:** confirmei App.js ESTAVEL (hash idempotente em 3s + monitor) antes de re-ancorar, pra nao ancorar numa linha que a concorrente ainda move.

**EXT-REP2-01 (S):** NAO re-implementado — ja shipped no commit af3b693 (2026-07-07, kAnonymizeRegionRecords k=5, provado por reports.test.js:106-155). Agent B corretamente parou (circuit-breaker re-propose-a-done-fix). Marcado shipped no doc.

**Gate (verde LIDO este turno, serializado 1x):** `npm run lint` 0 errors · frontend vitest (forks/no-parallelism, max-old-space 4096) **1433/1433** · `npm run fitness` **OK** (23 FF2 baseline-allowlisted incl. o constructor re-ancorado) · `npm run build` 13 rotas · `npm run smoke200` **16/16** · `cd asaas-backend && npm test` **112/112**.

**Isolamento da concorrente (H3/D23):** 7 commits staged por path explicito, um por concern. Nenhum arquivo da sessao concorrente (sw.js, version.json, App.js-alheio? nao — App.js entrelacado stagiado inteiro so porque o gate passa com o arquivo todo; Apoiadores.js, sponsors.js, strings.page.js, CreatorsMapaFome del, test/sponsors.test.js, PRIORIDADES.md, e2e, .bat) commitado — provado por `git log --name-only e39cf61..HEAD` + hash-object antes/depois (4 arquivos de conteudo UNCHANGED; sw.js/version.json mudaram so por re-stamp do meu prebuild, NAO staged, ficam no working-tree da concorrente).

**Milestones:** 7 flipados pending->shipped no MILESTONES_EXTENDED.yaml (com shipped_note citando o commit + gate verde), via edits cirurgicos (um rewrite via yaml.parseDocument foi REVERTIDO por reformatar o doc inteiro 2349->4320 linhas — edit minimo restaurado). Parser: 168/89/0 dups, shipped 2->9, pending 144->137. status.

**Commit deste log-update:** roteado pro agent_git-commit-specialist, staging so MILESTONES_EXTENDED.yaml + CHANGES.md. Sem push/PR/bump.

---

## 2026-07-10-AD — ONDA 3 shipped (6 milestones, 5 commits)

**Comando:** "onda 3" -> implementar a ONDA 3 (a11y/i18n ultimo-metro) da PRIORIDADES.md.

**Metodo:** 3 agents em PARALELO agrupados por arquivo disjunto: (1) A11Y+FOCUSTRAP (useFocusTrap + sheets), (2) assinar-i18n (I18N-04+CURRENCY, shard strings.assinar.js LIMPO), (3) hreflang (layouts/sitemap/metadata). ARIALIVE/I18N-03/SHARELOC SEGURADOS pra depois (tocam strings.page.js, sujo pela concorrente). Os 3 agents foram PARADOS pelo usuario mid-work.

| id | tier | fix | arquivos | commit |
|---|---|---|---|---|
| EXT-A11Y-01 | S- | util compartilhado `useFocusTrap(active, ref)` — Tab/Shift+Tab cicla no dialog aria-modal (re-query de focaveis por keydown, empty-set swallow, tabindex=-1 excluido) | useFocusTrap.js (novo), ReportSheet/PinDetailSheet/NotificationPrefs/PetDetailSheet/PetReportSheet + 2 hooks pets, +test | 7ee3eeb |
| EXT-FOCUSTRAP-01 | S- | fundido com A11Y-01 (mesmo tema Tab-trap) | (idem) | 7ee3eeb |
| EXT-I18N-04 | S- | 4 mensagens de falha de pagamento hardcoded pt-BR -> chaves t() em strings.assinar.js (12 locales, paridade) | asaasSubscriptionClient.js, strings.assinar.js | 5388046 |
| EXT-CURRENCY-01 | S- | comentario BRL-only-by-design nas strings.assinar.js (o 'R$' bakeado e intencional, Asaas so-BRL) | strings.assinar.js | 5388046 |
| EXT-CURRENCY-02 | S- | erros de validacao -> t(); CPF/CNPJ-condicional-ao-pais deixado HUMAN-GATED (depende da regra Asaas) | asaasSubscriptionClient.js | 5388046 |
| EXT-HREFLANG-01 | S- | og:locale + og:locale:alternate pros locales suportados (minimo honesto pro client-swap single-URL); hreflang-por-rota (/es /ar) HUMAN-GATED, deferido com comentario | structuredData.js, structuredData.test.js, layout.js, sitemap.js | 7f4fd51 |

**RECUPERACAO DE TREE QUEBRADO (os 3 agents pararam mid-work):**
1. **Build-break:** o agent hreflang importou `SUPPORTED_LOCALES` de engine.js (que e `'use client'`) pra dentro do structuredData.js (server metadata) — arrastou o boundary client->server e quebrou `next build` na coleta de /imprensa com `d.SUPPORTED_LOCALES.filter is not a function`. FIX: extrai DEFAULT_LOCALE+SUPPORTED_LOCALES pra `i18n/locales.js` React-free; engine.js re-exporta (todos os 12 consumidores client intactos); structuredData.js importa do modulo puro. Commit d9fbbab (build unblock).
2. **Bug real do focus-trap:** `button[tabindex=-1]` casava `button:not([disabled])` no selector e entrava no ciclo como phantom last -> o wrap mirava o elemento errado. FIX: `getFocusable` exclui `tabindex="-1"` explicitamente. 7/7 focusTrap tests.
3. **FF2 red por ancora deslocada:** o wiring do useFocusTrap cresceu NotificationPrefs/PinDetailSheet/ReportSheet e deslocou as 3 entradas line-anchored do FF2_BASELINE (45->46, 84->85, 29->30). Re-ancoradas (mesmo debt ja-allowlistado, ancora+LOC atualizados). Commit 5dbb729.

**SPLIT (5 commits, decidido pelo commit-specialist):** d9fbbab feat(i18n) locale SOT React-free · 7ee3eeb feat(a11y) focus-trap · 5dbb729 chore(fitness) re-anchor · 5388046 fix(i18n) assinar errors · 7f4fd51 feat(seo) og:locale. structuredData.js ficou INTEIRO no commit de hreflang (7f4fd51), NAO no build-fix — o import de SUPPORTED_LOCALES la e NOVO (so alimenta ogLocaleFor), nao um swap pre-existente; o build-fix real e so a extracao locales.js+engine.js.

**Gate (verde LIDO este turno, serializado 1x):** `npm run lint` 0 errors · full vitest **1445/1445** (103 files) · `npm run fitness` **OK** (23 FF2 + 3 FF1 baseline) · `npm run build` 13 rotas (apos rm -rf .next pra limpar o cache do build falho) · `npm run smoke200` **16/16** · focusTrap.dom.test.js **7/7**.

**Isolamento da concorrente (H3/D23):** 5 commits staged por path, um por concern. strings.page.js (sujo, da concorrente) NAO commitado — os itens que o tocariam ficaram SEGURADOS. Provado por hash-object: sponsors.js/strings.page.js/test-sponsors UNCHANGED antes/depois.

**Milestones:** 6 flipados pending->shipped (com shipped_note citando commit + gate + as partes HUMAN-GATED). Parser: 168/89/0 dups, shipped 9->15, pending 137->131.

**SEGURADOS pra ONDA 3-b (dependem do strings.page.js estabilizar):** EXT-ARIALIVE-01 (LiveAnnouncer -> t()), EXT-I18N-03 (toast SW update -> t()), EXT-SHARELOC-01 (texto share WhatsApp -> t() + encodeURIComponent). Todos gravam chaves em strings.page.js, hoje sujo pela sessao concorrente.
